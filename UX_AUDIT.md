...
| Scan | Late (for_review) scans shown as plain green success — no distinct BLUE "Recorded — for review" state; no per-outcome sound; RFID wedge focus not restored after dialogs/navigation/app resume; back arrow 18px no-tooltip target | §6, §4 | ☐ |
...
- [ ] Sign-out flows match §2 on all clients, incl. checker unsynced-scans warning
- [ ] Every password field toggles visibility
- [ ] Every list has loading/empty/error states (error+Retry demonstrable offline)
- [ ] Scan feedback per §6 (BLUE for-review state, sound+haptic per outcome — full custom audio →TODO)
- [ ] System back never dead-ends (verified: all pushed screens pop; sign-out resets stack)
- [ ] Zero open violations or TODO.md entry with reason (see →TODO rows)
